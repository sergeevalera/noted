...
- Re-enable grammar in `languages/obsidian-md/config.toml` and `extension.toml`
...
2. **Tree-sitter grammar not generated:** `grammars/tree-sitter-obsidian-md/src/` must contain
...
3. **Tree-sitter conflict:** Zed already has a built-in `tree-sitter-markdown`.
   Our grammar registers as a separate language (`obsidian-md`), not overwriting standard Markdown.
